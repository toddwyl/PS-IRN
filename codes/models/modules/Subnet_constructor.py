...
class ResidualDenseBlock(nn.Module):
    def __init__(self, channel_in, channel_out, init='xavier', gc=32, bias=True, instance_norm=False):
        ...
        self.weight_conv = nn.Conv2d(channel_in, channel_out, 3, 1, 1, bias=bias)
        self.conv1 = nn.Conv2d(channel_in, gc, 3, 1, 1, bias=bias)
        self.conv2 = nn.Conv2d(channel_in + gc, gc, 3, 1, 1, bias=bias)
        self.conv3 = nn.Conv2d(channel_in + 2 * gc, gc, 3, 1, 1, bias=bias)
        self.conv4 = nn.Conv2d(channel_in + 3 * gc, gc, 3, 1, 1, bias=bias)
        ...
        if init == 'xavier':
            mutil.initialize_weights_xavier(
                [self.weight_conv, self.conv1, self.conv2, self.conv3, self.conv4], 0.1)
        else:
            mutil.initialize_weights(
                [self.weight_conv, self.conv1, self.conv2, self.conv3, self.conv4], 0.1)
        mutil.initialize_weights(self.conv5, 0)
    ...
    def forward(self, x):
        ...
        x5 = self.conv5(torch.cat((x, x1, x2, x3, x4), 1))
        if self.instance_norm:
            return self.norm(x5*torch.sigmoid(xr))
        else:
            return x5*torch.sigmoid(xr)
