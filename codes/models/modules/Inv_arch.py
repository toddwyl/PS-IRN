...
class InvBlockExp(nn.Module):
    def __init__(self, subnet_constructor, channel_num, channel_split_num, clamp=1.):
        ...
        self.clamp = clamp
        if isinstance(subnet_constructor, dict):
            self.F = subnet_constructor['F'](self.split_len2, self.split_len1)
            self.G = subnet_constructor['G'](self.split_len1, self.split_len2)
            self.H = subnet_constructor['H'](self.split_len1, self.split_len2)
            self.K = subnet_constructor['K'](self.split_len2, self.split_len1)
        else:
...
class InvRescaleNet(nn.Module):
    def __init__(self, channel_in=3, channel_out=3, subnet_constructor=None, block_num=[],
                 down_num=1, scale=2, preprocess_op='Haar', clamp=1.):
        ...
        for i in range(down_num):
            ...
            for j in range(block_num[i]):
                b = InvBlockExp(subnet_constructor,
                                current_channel, channel_out, clamp=clamp)
                operations.append(b)
