...
from geo_ensemble import ensemble_augment, ensemble2origin
import numpy as np
    ...
    for test_loader in test_loaders:
        ...
        for data in test_loader:
            if ensemble:
                ...
                datas_LQ = ensemble_augment(data['LQ'])
                datas_GT = ensemble_augment(data['GT'])
                datas = []
                for d_lq, d_gt in zip(datas_LQ, datas_GT):
                    d = data.copy()
                    d['LQ'] = d_lq
                    d['GT'] = d_gt
                    datas.append(d)
            ...
            for idx, d in enumerate(datas):
                ...
                model.feed_data(d)
                model.test(opt['test_LR_ref'])
                if idx == 0:
                    visuals = model.get_current_visuals()
                visuals_ensemble = model.get_current_visuals()
                sr_img = util.tensor2img(visuals_ensemble['SR'])  # uint8
                sr_imgs.append(sr_img)
            sr_imgs = ensemble2origin(sr_imgs)
            sr_img = np.sum(sr_imgs, axis=0)//len(sr_imgs)
            ...
            if suffix:
                save_img_path = osp.join(
                    dataset_dir, img_name + suffix + '_GT.png')
            else:
            ...
            if suffix:
                save_img_path = osp.join(
                    dataset_dir, img_name + suffix + '_LR.png')
            else:
            ...
            if suffix:
                save_img_path = osp.join(
                    dataset_dir, img_name + suffix + '_LR_ref.png')
            else:
            ...
            if gt_img.shape[2] == 3:  # RGB image
                ...
                logger.info(
                    ...
                    .format(img_name, psnr, ssim, psnr_y, ssim_y, psnr_lr, ssim_lr, psnr_y_lr,
                            ssim_y_lr))
            else:
                logger.info(
                    '{:20s} - PSNR: {:.6f} dB; SSIM: {:.6f}. LR PSNR: {:.6f} dB; SSIM: {:.6f}.'.format(
                        img_name, psnr, ssim, psnr_lr, ssim_lr))
        ...
        logger.info(
            ...
            .format(test_set_name, ave_psnr, ave_ssim, ave_psnr_lr, ave_ssim_lr))
        if test_results['psnr_y'] and test_results['ssim_y']:
            ave_psnr_y = sum(test_results['psnr_y']) / len(test_results['psnr_y'])
            ave_ssim_y = sum(test_results['ssim_y']) / len(test_results['ssim_y'])

            ave_psnr_y_lr = sum(
                test_results['psnr_y_lr']) / len(test_results['psnr_y_lr'])
            ave_ssim_y_lr = sum(
                test_results['ssim_y_lr']) / len(test_results['ssim_y_lr'])
            logger.info(
